...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'staff') {
    header("Location: login.php?error=Please+login+as+staff");
    exit();
}
?>
...
// Get visitor ID
$sqlforv = "SELECT staff_id from user where user_id = '$userId'";
// $stmt = $conn->prepare($sqlforv);
// $stmt->bind_param("s", $userId);
// $stmt->execute();
$resultv = mysqli_query($conn, $sqlforv);
if (mysqli_num_rows($resultv) > 0) {
    $rowv = mysqli_fetch_assoc($resultv);
}
$visitor = $rowv['staff_id'];
// Prepare SQL to get the latest request
$sql = "SELECT status FROM requests WHERE professional_id = '$visitor' order by request_id desc";
