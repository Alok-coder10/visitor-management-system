...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'visitor') {
    header("Location: login.php?error=Please+login+as+visitor");
    exit();
}
?>
...
// Get visitor ID
$sqlforv = "SELECT visitor_id from user where user_id = '$userId'";
...
$visitor = $rowv['visitor_id'];
// Prepare SQL to get the latest request
$sql = "SELECT * FROM requests WHERE professional_id = '$visitor' order by request_id desc";
